<script lang="ts">
	import { enhance } from '$app/forms';
	let signerName = $state('');
	let signers = $state(['Emily Carter', 'Sophia Hayes', 'Rachel Bennett']);
	let { stop } = $props();

	function addDemoSigner() {
		signers.push(signerName);
	}
</script>

<div class="main">
	<h3 class="stopTitle">{stop.custName}</h3>
	<div class="stopData">
		<div class="left">
			<p>
				Address: <span>{stop.address}</span>
			</p>
			{#if stop.suite !== ''}
				<p>
					Suite: <span>{stop.suite}</span>
				</p>
			{/if}

			<p>
				City: <span> {stop.city}</span>
			</p>
			<p>
				Delivery Location: <span>{stop.deliveryLocation}</span>
			</p>
			{#if stop.notes !== ''}
				<p>
					Notes: <span>{stop.notes}</span>
				</p>
			{/if}
		</div>
		<div class="right">
			<div class="signerList">
				<p>Past Signers</p>
				<ul>
					{#each signers as signer, index (signer)}
						<li>{signer}</li>
					{/each}
				</ul>
			</div>

			<input
				class="addSignerText"
				name="signersName"
				placeholder="Signer Name"
				type="text"
				bind:value={signerName}
			/>
			<input class="addSignerBtn" value="Add Signer" onclick={addDemoSigner} />
		</div>
	</div>

	<div class="linkArea">
		<a class="updateLink" href={`/demoupdatenote/${stop._id}`} onclick={() => false}>Update Note</a>
	</div>
</div>

<style>
	.main {
		width: 50%;
		padding-left: 2rem;
		margin: 1rem 0;
		border-radius: 8px;
		background-color: #1e1f23;
		box-shadow: 5px 5px 10px rgba(95, 95, 95, 0.2);
	}
	.main p {
		font-weight: bold;
		color: #fff;
	}
	.main span {
		font-weight: normal;
	}

	.stopTitle {
		text-align: center;
		font-size: 1.5rem;
		color: #fff;
	}

	.left {
		padding: 0 1rem;
		width: 50%;
	}
	@media (max-width: 676px) {
		.left {
			width: 100%;
		}
	}

	.right {
		display: flex;
		flex-direction: column;
		justify-content: space-between;
		padding: 0 1rem;
		width: 50%;
	}
	@media (max-width: 676px) {
		.right {
			width: 100%;
		}
	}

	.signerList ul {
		list-style-type: none;
	}

	.signerList li {
		color: #fff;
	}

	.stopData {
		display: flex;
		margin-right: 2rem;
	}
	@media (max-width: 676px) {
		.stopData {
			flex-direction: column;
		}
	}

	.linkArea {
		text-align: center;
	}

	.updateLink {
		display: inline-block;
		text-decoration: none;
		font-weight: bold;
		color: white;
		overflow: hidden;
		background-color: #5f48d9;
		margin: 1rem 0;
		padding: 0.5rem 1rem;
		border: 1px solid #5f48d9;
		box-shadow: 5px 5px 10px rgba(95, 95, 95, 0.3);
		border-radius: 5px;
	}

	.addSignerBtn {
		width: 5rem;
		display: inline-block;
		text-decoration: none;
		font-weight: bold;
		color: white;
		overflow: hidden;
		background-color: #5f48d9;
		margin: 1rem 0;
		padding: 0.5rem 1rem;
		border: 1px solid #5f48d9;
		box-shadow: 5px 5px 10px rgba(95, 95, 95, 0.3);
		border-radius: 5px;
	}

	.addSignerText {
		padding: 0.5rem;
		margin-right: 0.5rem;
		width: 50%;
	}
</style>
